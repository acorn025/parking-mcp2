...
def test_hwaseong_parking():
    """수원 화성행궁 근처 주차장 검색 테스트"""
    print("=" * 70)
    print("수원 화성행궁 근처 주차장 검색 통합 테스트")
    print("=" * 70)
    ...
    try:
        ...
        for i, parking in enumerate(parking_list, 1):
            ...
            realtime_info = None
            if region == "seoul":
                print(f"  [{i}] {parking.get('name')} - 서울 지역: 실시간 정보 조회 중...")
                realtime_info = _get_realtime_info_seoul(
                    parking.get("name", ""),
                    address
                )
            elif region == "gyeonggi":
                print(f"  [{i}] {parking.get('name')} - 경기 지역: 실시간 정보 조회 중...")
        ...
        for i, parking in enumerate(formatted_parkings[:10], 1):  # 상위 10개만
            ...
            region_display = "서울" if "서울" in parking.get('address', '') else ("경기" if "경기" in parking.get('address', '') else "기타")
            print(f"지역: {region_display}")
            ...
            if parking.get('available_spots') is not None:
                ...
                if parking.get('current_spots') is not None:
                    print(f"  현재 주차 대수: {parking.get('current_spots', 'N/A')}")
            else:
